...
bool is_avl_helper(avl_node_t *root){
    if(root == NULL){
        return true;
    }
                ...
                int (*cmp)(bag_elem_t, bag_elem_t))
{
    ...
    if (! *root) {
        ...
        /* Insert into the subtree with smaller height. */
        if (HEIGHT((*root)->left) < HEIGHT((*root)->right))
            inserted = avl_insert_norot(&(*root)->left, elem, cmp);
        else
...
{
    bool removed = false;
    if(((*cmp)(elem, (*root)->elem)) < 0){
        if ((removed = avl_remove_2(&(*root)->left, elem, cmp))){
            /* Check if the subtree needs rebalancing; update its height. */
            if (HEIGHT((*root)->right) > HEIGHT((*root)->left) + 1)
                avl_rebalance_to_the_left(root);
            else
        ...
        if ((removed = avl_remove_2(&(*root)->right, elem, cmp))){
            /* Check if the subtree needs rebalancing; update its height. */
            if (HEIGHT((*root)->left) > HEIGHT((*root)->right) + 1)
                avl_rebalance_to_the_right(root);
            else
...
bool bag_remove_2(bag_t *b, bag_elem_t elem)
{
    if (avl_remove_2(&b->root, elem, b->cmp)) {
        b->size--;
        return true;
    } else {
        return false;
